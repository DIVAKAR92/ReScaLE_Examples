...
//============================ Phi function for bimodal example ===========================
// [[Rcpp::plugins(cpp11)]]
// [[Rcpp::export]]   
double bimodal_phi_norm(double x, double l_1 =-2.5, double l_2=2.5, double p = 0.33){
  double res = 0;
  double den = (1-p)*exp(-pow((x-l_2),2)/2) + (p)*exp(-pow((x-l_1),2)/2);
  res = (1-p)*pow((x-l_2),2)*exp(-pow((x-l_2),2)/2) + (p)*pow((x-l_1),2)*exp(-pow((x-l_1),2)/2);
  return res/(2*den);
}
...
//============================ upper bound function for bimodal example =================

double M_bimodal(vector<double> l, vector<double> u){
  double res = 0;
  res = maxi(bimodal_phi_norm(l[0]),bimodal_phi_norm(u[0]));
  res = maxi(res, 4);
  return res;
}
...
std::vector<std::vector<double>> rescale_layered_mult(double max_time, std::vector<double> W_s,
  ...
  while (cur_time < max_time){
    ...
    if(runif(1)[0] <= (1 - prior_C/(prior_C+t_kill))){
      ...
      for(int i=0; i<dim; i++){
        // reg_val[i] = 0.3*rnorm(1,-2.5,2)[0] + 0.7*rnorm(1,2.5,2)[0];
        // reg_val[i] = 0.33*rnorm(1,-2.5,1)[0] + 0.67*rnorm(1,2.5,1)[0];
        reg_val[i] = runif(1,-10,10)[0];
      }
    }
    s = t_kill;
    W_s = reg_val;
