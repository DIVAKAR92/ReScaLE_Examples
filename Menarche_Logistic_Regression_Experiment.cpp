...
// logistic regression model on the Menarche data. Note that the data has been hard
// coded in this example to enhance efficiency of the code. 
// [[Rcpp::plugins(cpp11)]]
// [[Rcpp::export]] 
vector<double> menarche_derivaties(vector<double> x){
  /*
  Input:
  --------------------------------------------------------------
  ...
  --------------------------------------------------------------
  */
  // X is the normalised covariate (i.e. Age) based on the Menarche data 
  vector<double> X = {-1.63291386,-1.28256859,-1.15294085,-1.06535453,-0.97776821,-0.89018190,-0.80259558,-0.71500926,
                      -0.62742295,-0.53983663,-0.45225031,-0.36466400,-0.27707768,-0.18949136,-0.10190505,-0.01431873,
                      0.07326759,0.16085390,0.24844022,0.33602654,0.42361285,0.51119917,0.59878549,0.68637180,
                      1.29947602};
  // N is the total number of people considered in the corresponding age bracket, refer to 
  // the Menarche data for more information
  vector<double> N ={376,200,93,120,90,88,105,111,100,93,100,108,99,106,105,
                     117,98,97,120,102,122,111,94,114,1049};
  // n is the number of people who reach Menarche out of N 
  vector<double> n = {0,0,0,2,2,5,10,17,16,29,39,51,47,67,81,88,79,90,113,
                      95,117,107,92,112,1049};
  ...
  for(int i=0; i<25; i++){
    // calculating the probability corresponding to a given Age group
    plogi = exp(x[0]+x[1]*X[i])/(1+exp(x[0]+x[1]*X[i]));
    // calculation of the first derivatives 
    f_x += n[i] - N[i]*plogi;
    f_y += (n[i] - N[i]*plogi)*X[i];
    // Calculation of the hessian matrix 
    f_xx += -N[i]*plogi*(1-plogi);
    f_xy += -N[i]*plogi*(1-plogi)*X[i];
    f_yy += -N[i]*plogi*(1-plogi)*X[i]*X[i];
  }
...
// [[Rcpp::plugins(cpp11)]]
// [[Rcpp::export]]
double menarche_kappa_trans(vector<double> z){
  // An estimate of the posterior mode
  vector<double> map = {1.410426, 4.658172};
  // A transformation matrix which is used to transform the posterior distribution
  vector<vector<double>> T = {{0.08027078,0},{0.10404352, 0.1322499}};
  ...
  vector<double> d = menarche_derivaties(xt);
  // res[0] = f_x; res[1] = f_y; res[2] = f_xx; res[3] = f_xy; res[4] = f_yy;
  ...
  return ((res/(2)) + 1.2)/(pow(tf, 2));
}


// [[Rcpp::plugins(cpp11)]]
// [[Rcpp::export]]
double M_menarche (std::vector<double> lower, std::vector<double> upper){
  ...
  values[0] = menarche_kappa_trans(v1); values[1] = menarche_kappa_trans(v2);
  values[2] = menarche_kappa_trans(v3); values[3] = menarche_kappa_trans(v4);
  // find the maximum value 
  double max_val = 0;
  std::vector<double>::iterator it = max_element(values.begin(),values.end());
  max_val = *(it);
  return max_val;//0.56;//0.001;//1.5; //25.5
}
